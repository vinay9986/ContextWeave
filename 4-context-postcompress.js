...
function buildContextPack() {
  const pinned = asIssueList(runBd(['list', '--pinned', '--limit', '5']) || []);
  const decisions = asIssueList(runBd(['list', '--type', 'decision', '--limit', '3']) || []);
...
const response = output.emitContext({
  ...
  systemMessage: 'Beads rehydrated (post-compress).',
});
